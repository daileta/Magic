...
abstract class ServerTickCoordinator extends ActiveAbilityEffectService {
	static void onEndServerTick(MinecraftServer server) {
		...
		GreedAbilityRuntime.onEndServerTick(server, currentTick);
	...
	static void onPlayerDeath(ServerPlayerEntity player) {
		...
		GreedAbilityRuntime.clearAllRuntimeState(player);
		...
		GreedAbilityRuntime.onPlayerDeath(player);
	}
	...
	static void regenerateMana(ServerPlayerEntity player, int currentMana) {
		if (GreedAbilityRuntime.blocksManaRegen(player)) {
			return;
		}
