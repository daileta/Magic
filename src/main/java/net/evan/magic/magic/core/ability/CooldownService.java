...
abstract class CooldownService extends DomainBlockService {
	...
	static int cooldownRemaining(ServerPlayerEntity player, MagicAbility ability, int currentTick) {
		...
		return switch (ability) {
			...
			case APPRAISERS_MARK, TOLLKEEPERS_CLAIM, KINGS_DUES, BANKRUPTCY -> GreedAbilityRuntime.cooldownRemaining(player, ability, currentTick);
	...
	static void startAbilityCooldownFromNow(UUID playerId, MagicAbility ability, int currentTick) {
		...
		if (
			...
			GreedAbilityRuntime.startAbilityCooldownFromNow(playerId, ability, currentTick);
			return;
		}
